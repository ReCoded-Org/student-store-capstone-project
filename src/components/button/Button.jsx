export default function Button({ buttonStyle, type, text, handleClick }) {
    ...
    const signInBtnSignUpPage =
        "h-9 w-36 m-1 p-1 rounded-lg border-2 bg-purple font-extralight text-sm text-white shadow-sm";
    const signUpBtnSignUpPage =
        "h-10 w-40 m-1 p-1 rounded-lg border-2 bg-purple font-extralight text-[15px] text-white shadow-sm";
    const listItemBtn =
        "h-12 w-72 md:h-14 md:w-80 lg:h-14 lg:w-96 m-1 p-1 rounded-lg border-2 bg-pumpkin font-md text-[17px] text-white shadow-sm";
    const donateBtn =
        "flex items-center justify-center w-full h-8 px-12 text-xs rounded-lg border-2 bg-pumpkin font-extralight text-white shadow-sm";
            ...
            <button
                className={
                    buttonStyle === "orangeSignIn"
                        ...
                        ? signInBtnSignUpPage
                        : buttonStyle === "listItemBtn"
                        ? listItemBtn
                        : buttonStyle === "donateBtn"
                        ? donateBtn
                        : buttonStyle === "saveChangesBtn"
                        ? saveChangesBtn
                        : null
                }
                onClick={handleClick}
